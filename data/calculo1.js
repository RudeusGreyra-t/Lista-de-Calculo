...
{materia:"Derivadas", subtopico:"Regra da cadeia", secId:"derivadas", exercicios:[
  {id:"P1-OFI-003", numero: 3, origem:"Prova antiga", oficial:"⭐ P1 Oficial", dificuldade:"Médio", relevancia:"Muito Alta",
...
{materia:"Integrais", subtopico:"Substituição simples", secId:"integrais", etapa: "P1", exercicios:[
  {id:"P1-OFI-005", numero: 5, origem:"Prova antiga", oficial:"⭐ P1 Oficial", dificuldade:"Médio", relevancia:"Muito Alta",
...
{materia:"Integrais", subtopico:"Integração por Partes", secId:"integrais", etapa: "P2", exercicios:[
  {id:"INT-PAR-001", origem:"Dado em aula[cite: 9]", dificuldade:"Médio", relevancia:"Alta",
   enunciado:"Calcule a integral utilizando a técnica de integração por partes: $$\\int x e^x \\, dx$$",
   resposta:"$$\\int x e^x \\, dx = x e^x - \\int e^x \\, dx = x e^x - e^x + C, \\quad C \\in \\mathbb{R}$$"},
  {id:"INT-PAR-002", origem:"Dado em aula[cite: 9]", dificuldade:"Médio", relevancia:"Alta",
   enunciado:"Calcule a integral: $$\\int x \\sin(x) \\, dx$$",
   resposta:"Fazendo \\( f(x) = x \\) e \\(g'(x) = \\sin(x) \\):<br>$$\\int x \\sin(x) \\, dx = -x \\cos(x) - \\int (-\\cos(x)) \\, dx = -x \\cos(x) + \\sin(x) + C, \\quad C \\in \\mathbb{R}$$"},
  {id:"INT-PAR-003", origem:"Dado em aula[cite: 9]", dificuldade:"Difícil", relevancia:"Alta",
   enunciado:"Calcule a integral: $$\\int \\sin^2(x) \\, dx$$",
   resposta:"$$\\int \\sin^2(x) \\, dx = -\\sin(x)\\cos(x) + \\int \\cos^2(x) \\, dx$$<br>Substituindo \\(\\cos^2(x) = 1 - \\sin^2(x) \\):<br>$$2 \\int \\sin^2(x) \\, dx = -\\sin(x)\\cos(x) + x$$<br>$$\\int \\sin^2(x) \\, dx = \\frac{-\\sin(x)\\cos(x) + x}{2} + C, \\quad C \\in \\mathbb{R}$$"},
  {id:"INT-PAR-004", origem:"Dado em aula[cite: 10]", dificuldade:"Médio", relevancia:"Alta",
   enunciado:"Calcule a integral: $$\\int x^2 e^x \\, dx$$",
   resposta:"$$\\int x^2 e^x \\, dx = x^2 e^x - 2 \\int x e^x \\, dx$$<br>Utilizando o resultado de integração por partes novamente:<br>$$x^2 e^x - 2(x e^x - e^x) + C = x^2 e^x - 2x e^x + 2e^x + C, \\quad C \\in \\mathbb{R}$$"},
  {id:"INT-PAR-005", origem:"Dado em aula[cite: 10]", dificuldade:"Fácil", relevancia:"Alta",
   enunciado:"Calcule a integral: $$\\int \\ln(x) \\, dx$$",
   resposta:"Fazendo \\(f(x) = \\ln(x) \\) e \\( g'(x) = 1 \\):<br>$$\\int \\ln(x) \\, dx = x \\ln(x) - \\int \\frac{1}{x} \\cdot x \\, dx = x \\ln(x) - x + C, \\quad C \\in \\mathbb{R}$$"},
  {id:"INT-PAR-006", origem:"Dado em aula[cite: 10]", dificuldade:"Difícil", relevancia:"Alta",
   enunciado:"Calcule a integral: $$\\int e^x \\cos(x) \\, dx$$",
   resposta:"Aplicando integração por partes ciclicamente, obtemos:<br>$$2 \\int e^x \\cos(x) \\, dx = e^x(\\cos(x) + \\sin(x))$$<br>$$\\int e^x \\cos(x) \\, dx = \\frac{e^x}{2}(\\cos(x) + \\sin(x)) + C, \\quad C \\in \\mathbb{R}$$"},
  {id:"INT-PAR-007", origem:"Dado em aula[cite: 10]", dificuldade:"Difícil", relevancia:"Alta",
   enunciado:"Calcule a integral: $$\\int x^3 e^{x^2} \\, dx$$",
   resposta:"Primeiro, faça a substituição \\( u = x^2 \\Rightarrow du = 2x \\, dx \\), reescrevendo como \\( \\frac{1}{2} \\int u e^u \\, du \\).<br>Aplicando integração por partes:<br>$$\\frac{1}{2}[u e^u - e^u] + C = \\frac{1}{2}x^2 e^{x^2} - \\frac{1}{2}e^{x^2} + C, \\quad C \\in \\mathbb{R}$$"}
]},
{materia:"Integrais", subtopico:"Somas Parciais (Frações Parciais)", secId:"integrais", etapa: "P2", exercicios:[
  {id:"INT-SOM-001", origem:"Dado em aula[cite: 11]", dificuldade:"Médio", relevancia:"Alta",
   enunciado:"Utilizando a técnica de somas parciais, calcule a integral: $$\\int \\frac{3}{(x-1)(x+2)} \\, dx$$",
   resposta:"Decompondo em frações parciais: \\(\\frac{3}{(x-1)(x+2)} = \\frac{A}{x-1} + \\frac{B}{x+2} \\).<br>Encontrando A e B: \\( 3 = A(x+2) + B(x-1) \\Rightarrow A=1, B=-1 \\).<br>Integrando:<br>$$\\int \\left(\\frac{1}{x-1} - \\frac{1}{x+2}\\right) \\, dx = \\ln(x-1) - \\ln(x+2) + C, \\quad C \\in \\mathbb{R}$$"},
  {id:"INT-SOM-002", origem:"Dado em aula[cite: 11]", dificuldade:"Médio", relevancia:"Alta",
   enunciado:"Calcule a integral por somas parciais: $$\\int \\frac{x}{x^2-9} \\, dx$$",
   resposta:"Fatorando o denominador: \\(x^2-9 = (x-3)(x+3) \\).<br>Decompondo: \\(\\frac{x}{(x-3)(x+3)} = \\frac{A}{x-3} + \\frac{B}{x+3} \\).<br>Encontrando as constantes: \\(A = \\frac{1}{2}, B = \\frac{1}{2} \\).<br>Integrando:<br>$$\\frac{1}{2}\\int \\frac{1}{x-3} \\, dx + \\frac{1}{2}\\int \\frac{1}{x+3} \\, dx = \\frac{1}{2}\\ln(x-3) + \\frac{1}{2}\\ln(x+3) + C, \\quad C \\in \\mathbb{R}$$"},
  {id:"INT-SOM-003", origem:"Dado em aula[cite: 11]", dificuldade:"Médio", relevancia:"Alta",
   enunciado:"Calcule a integral: $$\\int \\frac{2}{x^2-x-2} \\, dx$$",
   resposta:"As raízes do denominador são \\( x = 2 \\) e \\( x = -1 \\), logo \\(x^2-x-2 = (x-2)(x+1) \\).<br>Decompondo: \\(\\frac{2}{(x-2)(x+1)} = \\frac{A}{x-2} + \\frac{B}{x+1} \\).<br>Resolvendo para A e B: \\(2 = A(x+1) + B(x-2) \\Rightarrow A = \\frac{2}{3}, B = -\\frac{2}{3} \\).<br>Integrando:<br>$$\\frac{2}{3}\\ln(x-2) - \\frac{2}{3}\\ln(x+1) + C, \\quad C \\in \\mathbb{R}$$"}
]}
  ]
};